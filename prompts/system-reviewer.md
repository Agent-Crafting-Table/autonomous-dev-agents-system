...
1. Copy all prompt files to the public repo, replacing [Your Project]-specific names/paths with `[your-project]` placeholders:
   ...
   ```
   Then open each file in `/tmp/autonomous-dev-agents-system/prompts/` and replace: `vetware/` → `[your-project]/`, `[Your Project]` → `[Your Project]`, channel IDs → `YOUR_CHANNEL_ID`, `[your-github-username]` → `[your-github-username]`.
